...
export const LannaZodiacThaiMap = {
    ชวด: "หนู",
    เป้า: "วัว",
    ยี่: "เสือ",
    เม้า: "เถาะ",
...
const normalizeMod = (value, divisor) => ((value % divisor) + divisor) % divisor;
const zodiacFromChulasakarat = (chulasakarat) => {
...
const buildTransitionForYear = (gregorianYear) => {
    ...
    const thaloengSokMoment = new Date(BASE_THALOENG_SOK_UTC + diffDays * MS_PER_DAY + fraction * MS_PER_DAY);
...
export const lannaYearTransitions = Array.from({ length: 2100 - 1900 + 1 }, (_, index) => buildTransitionForYear(1900 + index));
export const lannaYearTransitionByGregorianYear = new Map(lannaYearTransitions.map((entry) => [entry.gregorianYear, entry]));
